...
- 2026-08-03: Native-host receipt validation now unions the seat-overlay gate
  with pattern scanning: private paths (including mid-string markers behind
  backticks or parentheses), secret-shaped values, and control characters fail
  closed across summaries, test names, and reported paths; adapter-written
  blocked details and Git-derived path lists are scrubbed before persisting;
  the validated summary is persisted as bounded evidence. 138 Python tests, 3 JavaScript
  tests, and the public-ready scan pass on the tree merged with the v2.1.x
  main.
