...
Pilot Puppy is Vidux's friendly front door for getting work from an outcome to
an honest result. It is designed for a person who wants one clear next choice,
not an agent dashboard or a model-picker ceremony.

## One product, three responsibilities

- **Vidux** keeps the durable `PLAN.md`, Outcome / Ask / Steer state, proof, and
  resume boundary.
- **Pilot Puppy** is the right-hand driver: it turns an outcome into a bounded
...
Use `/pilot-puppy` in a skill host or `pilot-puppy` in a local shell. Existing
`/pilot` and `pilot` callers remain supported as compatibility aliases, and the
`pilot.*` schemas and `PILOT_*` environment names remain stable. This is an
additive brand migration; it does not rewrite history or create a second
runtime.

## Distribution without an install ritual

The canonical implementation is local because local custody is part of the
product promise. A future release may also expose thin, optional interfaces in
ChatGPT, Claude, or Cursor marketplaces so a non-technical person can discover
the workflow without cloning a repository or installing npm packages.
