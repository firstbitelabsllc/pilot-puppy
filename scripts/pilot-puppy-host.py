...
def validate_host_receipt(
    ...
    try:
        ...
        raise HostError("host_receipt_invalid", "host receipt summary is invalid") from None
    ...
    for item in tests:
        ...
        try:
            test_name = _receipt_text(item.get("name"), "test name", MAX_TEST_NAME_CHARS, private_values)
            safe_name = validate_public_text(test_name, "host receipt test name", maximum=MAX_TEST_NAME_CHARS)
        except RoutePacketError:
